...
import { currency, findDemoProductBySku } from '../../data/consumerCatalog'

interface DisplayItem { id: number; skuId: number; quantity: number; name: string; skuName: string; price: number | null; image: string }
...
async function load() {
  ...
  try {
    ...
    items.value = remote.map((item) => {
      const visual = findDemoProductBySku(item.skuId)
      return { ...item, name: visual.name, skuName: `SKU ${item.skuId} · 价格下单时确认`, price: null, image: visual.image }
    })
  } catch (error) {
    items.value = []
...
<template>
  <div class="bag-view">
    ...
    <main class="bag-shell">
      ...
      <section v-else-if="items.length" class="bag-layout">
        <div class="bag-items">
          <article v-for="item in items" :key="item.id" class="bag-item">
            <div class="bag-item__placeholder"><img :src="item.image" :alt="item.name"></div>
      ...
      <section v-else class="bag-empty"><p class="kicker">EMPTY FOR NOW</p><h2>这里还没有你的选择。</h2><p>从一件能让今天变好的物件开始。</p><RouterLink to="/stores/1001/products">去选购 →</RouterLink></section>
    </main>
  </div>
</template>
...
<style scoped>.bag-item__placeholder{overflow:hidden}.bag-item__placeholder img{width:100%;height:100%;object-fit:cover}</style>

...
import { getOrderDetail, mockPayOrder, orderErrorMessage, type CreateOrderResult, type OrderDetail } from '../../api/order'
  ...
  <div class="order-detail-view">
    ...
    <main class="detail-shell">
      ...
      <template v-else>
        ...
        <section v-if="isPendingPayment" class="payment-panel"><div><p class="section-label">MOCK PAYMENT</p><h2>确认这次选择</h2><p>这是课程项目的模拟支付，不会产生真实扣款。</p></div><button type="button" :disabled="paying" @click="pay">{{ paying ? '正在确认…' : `模拟支付 ${currency(displayAmount)}` }}</button><p v-if="payError" class="payment-error">{{ payError }}</p></section>
      </template>
    </main>
  </div>
</template>

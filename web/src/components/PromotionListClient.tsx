...
import { promotionPhase } from "@/lib/promotion-time";
import type { PublicPromotionList } from "@/lib/types";
...
export function PromotionListClient() {
  ...
  return (
    <main className="page-shell promotion-page">
      ...
      {failure && !data ? <div className="empty-state"><span className="eyebrow">SERVICE / UNAVAILABLE</span><h2>活动暂时无法读取。</h2><p>{failure}</p><button className="button" onClick={() => window.location.reload()} type="button">重新读取</button></div> : null}
      {failure && data ? <p className="form-error">活动状态暂时无法刷新，以下是上次读取的结果：{failure}</p> : null}

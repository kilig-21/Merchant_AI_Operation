...
- 当前步骤：步骤 16，消费者端的第一个完整购物流程页面闭环。
- 今天目标：把已有的商品浏览、商品详情和购物袋，接到真实下单、订单查询与模拟支付页面。

### 今天学到什么

- 前端不能只看 HTTP 是否成功：本项目的业务失败也会放在统一响应的 `code` 和 `message` 里，所以订单 API 层需要先判断 `code === 0`，再把 `data` 交给页面；库存不足、重复支付等情况才能显示出后端的真实提示。
- “创建订单成功”与“详情接口随后可用”是两个独立时刻：购物袋页面先使用创建订单返回的 `orderId`、订单号、金额和到期时间进入详情；详情页再请求完整订单明细。这样用户不会因为一次页面跳转而丢掉刚刚下单成功的反馈。
- 订单列表不适合照搬后台表格：消费者优先关心状态、金额、下单时间和下一步动作，因此列表用轻分割线和状态标签，详情页把模拟支付作为唯一主操作，商品明细放在第二层。
- 鉴权失败不等于网络失败：本项目的 Security 层会直接返回 HTTP 401/403，而业务层错误使用统一 `ApiResponse.code`。Axios 响应层需要同时处理这两种来源，页面才能提示用户重新登录或说明权限不足。

### 今天遇到的问题

| 问题 | 出现场景 | 最后怎么处理 | 是否已理解 |
|---|---|---|---|
| 文档显示步骤 15 已完成，但当前 `feature/web-v2` 分支没有对应代码 | 对接订单列表、详情和模拟支付接口时，当前 `OrderController` 只有 `POST /api/orders` | 对照 `main` 的 `79963ab` 核对接口字段后先完成前端；真实浏览器联调等待合入该提交 | 是 |
| 演示购物袋不能当成真实订单 | 商品详情和购物袋原本在后端不可用时会回退到本地演示数据 | 保留演示浏览体验，但在结算时明确提示演示购物袋不能提交真实订单，避免伪造订单状态 | 是 |

### 验证记录

- `npm.cmd run build` 通过，类型检查和生产构建均成功。
- 未登录直接访问 `/orders` 会被前端路由保护送往 `/consumer/login`，并保留跳转前的目标地址。
- 尚未验证的部分：真实消费者账号下的下单、模拟支付、订单列表和详情，需要先把 `main` 合入当前前端分支。

### 明天衔接

- 合入 `main` 的订单后端提交后，用消费者账号完整验证：加入购物袋、提交订单、查看详情、模拟支付、回到订单列表确认状态为 `PAID`。
...
- 超时关单不是查到订单后直接改状态，而是通过 `PENDING_PAYMENT + expire_at <= now` 的条件更新抢占关单资格。
- 只有条件更新返回 `1` 的线程才能释放库存和写 `ORDER_CLOSE` 流水；返回 `0` 时说明订单已经被其他流程处理，必须直接结束。
- RabbitMQ 消费者和定时兜底任务虽然入口不同，但最终复用同一个 `OrderCloseService`，这样库存释放规则只有一份。
- 支付也增加 `expire_at > now` 条件，形成“未过期才能支付、已过期才能关单”的边界。
- `OutboxPublisher` 仍然是必要的：它每 5 秒扫描待发布事件并发送到 RabbitMQ；只把 `System.out.println` 换成 `@Slf4j`，不能删除发布逻辑。
- `cartItemId`、`orderId`、`orderNo` 分别属于购物车项、数据库订单和业务订单号，不能混用。

### 今天完成

- [x] 完成订单关闭 Service、RabbitMQ 关闭消费者和定时兜底任务。
- [x] 验证延迟消息关闭订单 `60`，并核对 `ORDER_CLOSE` 库存流水。
- [x] 验证兜底任务关闭订单 `62`，库存从 `19/1` 恢复为 `20/0`。
- [x] 验证过期订单 `61` 的支付请求被拒绝，状态仍为 `PENDING_PAYMENT`。
- [x] 验证正常订单 `63` 支付成功，状态为 `PAID`，锁定库存扣减为 `0`。
- [x] 将 `OutboxPublisher` 的标准输出改为 `@Slf4j`，并完成 Maven 编译验证。

### 今天遇到的问题与解决

| 问题 | 原因与解决 | 是否已理解 |
|---|---|---|
| 订单 2/3/45/61 兜底关单失败 | 历史订单状态和 SKU 汇总锁定库存不一致；保留数据证据，不强行释放库存 | 是 |
| 支付接口一直不允许 | 第一次把购物车项 ID 当成订单 ID；改用下单接口返回的 `orderId` | 是 |
| 同一个幂等 key 下单冲突 | 同一个 key 不能代表不同请求参数，改用新 key 测试 | 是 |
| 修改 Mapper 后方法报红 | 关单方法被误替换成重复的支付方法，已恢复并编译通过 | 是 |
| 日志输出方式 | `System.out` 换为 `@Slf4j` 的 `log.info/log.error`，异常对象放在最后 | 是 |

### 今天未完成

- [ ] 支付与关单并发竞争测试未执行。本次明确暂不实现，不能把它记录为已完成。
- [ ] 退避重试、失败队列消费和更完整的失败恢复策略没有展开，留到后续需要时处理。

### 侧边任务/对话补充记录

- 解释了 Mapper 中普通用户查询、MQ/系统内部查询和兜底扫描查询的区别。
- 解释了 `selectById` 为什么不带 `consumer_id`：它由系统内部任务使用，不属于用户权限查询。
- 解释了为什么旧订单暂时不删除：删除订单而不同时处理明细、库存流水和消息记录会产生新的数据不一致。
- 解释了 `@Scheduled(fixedDelay = 5000)`、Outbox 扫描和 RabbitMQ 发布之间的关系。
- 解释了日志占位符：每个 `{}` 按顺序接收后面的普通参数，异常对象通常作为最后一个参数输出堆栈。

### 截图记录

- 本次聊天附件已核对并记录：订单 `60/62` 关单结果、订单 `61` 过期支付拒绝、订单 `63` 正常支付、库存 `19/0` 和三类库存流水。
- 截图目前仍保留在聊天附件中，尚未复制到 `docs/images/day-19/`，因此不虚构具体归档文件名。
...
## Day 32：2026-09-02 / R3～R5 真实购物车、地址与跨店结算

### 今天学了什么

- 购物车列表不是只返回 `skuId + quantity` 就够了。页面能正确展示并判断能否结算，后端必须在同一份消费者隔离查询里提供 SKU、SPU、店铺、价格、可用库存和不可购买原因等可信快照。
- 跨店结算是一次父结算组加多笔子订单的原子操作。创建成功、取消、支付和库存不足都要同时观察父组、子订单、购物车、可用库存、锁定库存和幂等记录，不能只看 HTTP `200`。
- 幂等键绑定的是一次“相同语义的请求”。同一个键重试相同参数应复用结果；同一个键却换了地址或购物车项必须返回 `409`，新一轮真实下单要改用新的键。
- 分支不是实时共享文件夹：`feature/backend` 保存最新后端加旧前端，`feature/web-v2` 保存最新前端加旧后端。前端根据已确定的接口合同实现，但最终端到端效果必须把两个提交合并到同一个集成分支再验收。

### 今天完成

- 用户在 `feature/backend` 完成 `CartItemDetailVO`、购物车 Join 查询和结算组详情富化；后端提交为 `5fe0766 feat(cart): return real cart item details` 与 `3415ae9 feat(checkout): return order details in checkout group`。
- 真实购物车接口已返回两个商家的商品详情、实时库存和可购买状态；库存为 `0` 的商品明确返回 `purchasable: false`、`unavailableReason: 库存不足`。
- 使用两个购物车项创建结算组 `8`，得到两笔待支付子订单、总额 `398.00`；同一幂等键改参数的冲突路径已返回 `409`。
- 取消结算组 `8` 后，父组和子订单均为 `CANCELLED`，库存从锁定状态恢复为 `16/0`、`5/0`（可用库存/锁定库存）。
- 曾发现结算组详情中的 `items` 为空、`shippingAddress` 为 `null`；将子订单项和地址快照读取接回 `CheckoutGroupService` 后，详情已返回完整数据。
- 构造跨店库存不足后，`POST /api/checkouts` 返回 `409`，购物车保留，库存与锁定库存不变，且没有残留结算组、子订单或幂等记录；证明失败路径整体回滚。
- 恢复库存后创建结算组 `10` 并支付成功，父组和两笔子订单均为 `PAID`，库存为 `15/0`、`4/0`。
- Agent 在 `feature/web-v2` 完成真实结算页面接线：选择购物车项、地址选择、创建结算组、详情、刷新、模拟支付、取消和错误提示。用户已提交 `61191b8 feat(web): connect real checkout flow`；该分支的 lint、类型检查、4 项 Vitest 和生产构建均通过。

### 遇到的问题与处理

| 问题 | 原因与处理 | 是否已理解 |
|---|---|---|
| 支付接口一开始返回 500 | 请求路径误写成 `/mock-pays`，Spring 没有对应路由并尝试按静态资源处理；改为 `/mock-pay` 后接口成功 | 是 |
| 结算组详情缺商品项和地址 | 详情 Service 只组装了订单主信息；补调用订单项 Mapper 与地址快照 Service | 是 |
| 同一幂等键再次下单返回 409 | 新请求改变了购物车项/地址，和已保存请求指纹不一致；这是防止错误复用的正确结果 | 是 |
| 库存不足后担心有脏数据 | 不能只看 `409`；同时查询购物车、`product_sku`、`commerce_order`、`checkout_group` 和幂等表，确认均符合整体回滚预期 | 是 |
| 前后端分支彼此看不到最新源码 | Git 分支是提交快照，不会自动同步；前端以接口合同开发，最终需要集成分支合并验证 | 是 |

### 验收证据

- 已归档 9 张无可见凭据截图到 `docs/images/day-33-r3-r5/`：真实购物车、创建结算组、幂等冲突、取消详情、取消库存恢复、库存不足状态、失败后库存未变、支付详情和支付后库存扣减。
- 带可见 Authorization/Bearer Token 的 ApiFox 截图不复制进仓库；对应结果仅以本文和 `daily-plan.md` 的文字事实记录。
- 本日后端真实验收以 ApiFox 与 DataGrip 为准；前端分支验证的是静态检查、单测和生产构建。合并前不能把二者误写为单一浏览器端到端验收。

### 下一步

- 由用户建立集成分支，合并 `feature/backend` 与 `feature/web-v2`。
- 在集成分支用真实消费者会话完成浏览器级验收，并确认成功、取消和库存不足后页面状态与数据库状态一致。
- 文档与截图当前位于 `feature/backend`，由用户检查后自行提交和推送。

## Day 32：2026-09-02 / R1、R2 真实前端联调验收

### 今天学了什么

- Demo 数据只能服务于显式演示会话，不能在真实账号请求失败时作为“成功结果”回退；否则用户会把本地假数据误认为真实订单、购物车或商家数据。
- BFF 不只是转发请求：它还负责把后端 401 反映为前端会话失效，并让 403、409、503 保持可识别的 HTTP 语义，供页面显示正确状态。
- 公开店铺和商品的名称、价格、库存、归属必须来自后端接口；静态图片、渐变和色彩可以保留，但只能是展示资产，不能承载业务事实。
- 双商家验收不能只看“有商品”：还要确认商品在公开店铺列表、搜索、单店页和商品详情中的店铺归属一致。

### 今天完成

- 完成 R1：真实/Demo 会话边界、真实业务页禁止 Demo 降级、401 Cookie 清理，以及 401、403、409、503 的前端错误状态。
- 完成 R2：公开店铺目录、全站搜索、店铺页、店铺商品页和商品详情改接真实公开接口，并补齐加载、空结果、404 和服务不可用状态。
- 前端 Biome lint、TypeScript 类型检查、Vitest、Next.js production build 通过；后端健康检查为 `UP`。
- 使用已有 API 创建并上架商家 B 的最小测试商品，完成两家商家公开数据、搜索、店铺页和商品详情的真实联调。
- 验证无效会话 401、消费者访问商家接口 403、重复注册 409、后端不可达 503，以及非法店铺 404。

### 遇到的问题与解决

| 问题 | 原因与解决 | 是否已理解 |
|---|---|---|
| 消费者会话在本地 HTTP 下没有自动携带 Cookie | 生产模式 Cookie 的安全属性不会在普通 HTTP 验收请求中自动回传；验收时仅在内存请求中注入已登录消费者会话来验证 403，不显示或记录 Token | 是 |
| 公开验收一开始只有商家 A 商品 | R2 的验收需要至少两个 tenant；通过已有商家接口为商家 B 创建并上架一条最小测试商品后完成双商家验证 | 是 |
| 自动化浏览器无法启动 | 本机自动化环境缺少 Chrome；已完成 HTTP、BFF、构建和服务健康验证，但不把未取得的浏览器视觉截图写成已完成 | 是 |

### 侧边任务/对话补充记录

- 用户确认前端由 Agent 直接实现，后端继续采用“先解释、用户写、共同验收”的协作方式；R3 起先完成后端购物车详情接口合同，再接前端。
- 前端分支 R1/R2 已由用户提交并推送；切回后端分支后确认 `docs/daily-plan.md` 的新路线修改已经在提交 `adfddfd` 中，之前的 stash 与该提交内容相同，因此恢复后工作区保持干净。
- 商家 B 的最小联调商品保留在本地测试数据库，仅用于后续两商家公开目录与搜索验证；没有修改后端业务源码。

### 下一步

- 进入 R3：先设计并由用户实现 `CartItemDetailVO`、购物车 Join 查询与消费者隔离测试。
- 接口合同稳定后，前端将展示真实商品、SKU、店铺、单价、库存和不可购买原因；数量修改或删除失败时需回滚 UI，不能回退本地 Demo 数据。

## Day 32：2026-09-02 / R6 真实售后与商家审核集成验收

### 今天学了什么

- 售后“审核通过”与“退款成功”是两个不同的业务事实。R6 只展示审核结果、申请金额和商家备注；真实退款、资金流水、退货物流与完成态要留给后续版本单独建状态机。
- 售后详情的租户隔离不能只靠前端隐藏按钮。商家详情查询以当前商家 `tenant_id` 为条件，商家 B 访问商家 A 的申请得到“售后申请不存在”且 `data: null`，因此没有跨租户数据泄露。
- 单个功能分支中的前端构建通过，不等于前后端组合已经验证。必须把后端与前端提交合并到 `feature/integration`，再用真实浏览器会话验收页面、BFF 与后端的完整链路。

### 今天完成

- Agent 在 `feature/web-v2` 完成 R6 前端：真实售后申请、我的售后列表/详情、商家售后审核列表/详情、审核通过或拒绝及明确的失败提示。
- 用户提交前端变更 `1fde4f2 feat: implement R6 after-sales workflow`，并合并到 `feature/integration`，得到 `f5e1185 merge: integrate R6 after-sales web`。
- 在集合分支验证消费者提交的真实售后申请可被商家 A 审核通过，消费者刷新详情后仍能看到“审核通过”、金额、原因和商家备注。
- 验证商家 B 无法读取商家 A 的售后详情：HTTP/body `409`、“售后申请不存在”、`data: null`。
- R6 前端静态检查、4 项测试和生产构建通过；集成冒烟验收后集合分支工作区保持干净。

### 遇到的问题与处理

| 问题 | 原因与处理 | 是否已理解 |
|---|---|---|
| 初次浏览器脚本没有立即读到售后状态 | Next.js 页面先渲染壳层，再由客户端请求真实数据；测试需等待登录 Cookie 建立与“审核通过”元素实际出现，不能只在导航结束时立刻断言 | 是 |
| 商家 B 返回的是 `409` 而非 `404` | 当前后端把“售后申请不存在”作为业务异常映射为 `409`；关键安全结果是详情未返回、`data` 为 `null`。状态码语义若要调整，应在后端阶段单独讨论 | 是 |
| 启动前端后 `next-env.d.ts` 显示 `+1 -1` | Next.js 开发服务会把类型引用临时改到 `.next/dev`；停止服务后已还原为 `.next/types`，最终 Git 工作区干净，不是遗留的业务代码修改 | 是 |

### 侧边任务/对话补充记录

- 本地新建分支不会自动出现在远端；第一次执行 `git push -u origin <branch>` 才会创建远端分支并建立跟踪关系，之后使用 `git push` 即可。
- 未追踪文件只存在于本机工作区，尚不属于任何分支提交；`git add .` 会把当前目录下未被忽略的文件一并纳入暂存，因此提交前要先用 `git status --short` 核对范围。
- `feature/backend`、`feature/web-v2` 和 `feature/integration` 是不同的提交快照。集合分支用于合并和验收，不替代两个功能分支；R6 仍在前端分支开发并提交，再合入集合分支。
...
- R8 已完成；进入 R9 前先完成代码质量与分支收口，暂不新增部署代码。

## Day 33 加餐收口：进入 R9 前的代码质量检查

### 本次完成

- 以 `feature/integration` 的真实合并结果为基线复核 R8；后端、前端和集合分支均与对应远端同步，工作区初始干净。
- 后端 `mvn -q -DskipTests compile` 通过；前端 `npm run check`、`npm run test`（4 项）通过。
- 发现并修复既有 `CheckoutServiceTest` 的测试包归属和 mock 问题：测试直接调用 `CheckoutService` 的内部辅助方法，原测试包无法访问，且结算幂等 Mapper 未注入；将方法收紧为包内可见、测试归入 service 包并补齐 mock，生产代码没有对外暴露 public 辅助接口。
- 新增公开促销读取服务测试，覆盖服务器时间、活动列表、详情读取和不存在活动返回 `404`。

### 验证结果与剩余环境问题

- 后端 `test-compile`、定向服务/结算测试通过；前端生产构建通过，并包含 `/promotions` 与 `/promotions/[activityId]`。
- 完整 `mvn test` 已重新执行：72 个测试、0 个失败、45 个错误；错误均发生在集成测试启动阶段，根因是 Docker MySQL 拒绝当前 root 密码，不能归因于 R8。
- 本次不新增 R9 部署代码，也不做新的真实抢购写入。

### 侧边任务/对话补充记录

- 用户决定跳过 R8 现场写入验收，改由 Agent 通过代码、测试、编译和构建检查完成进入 R9 前的收口。后续仍要把“代码验证通过”和“真实环境写入验收通过”区分记录。

## Day 33：2026-09-04 / R8 限量促销真实联调（加餐）

### 今天学了什么

- 活动“列表”和“详情”承担不同职责：列表用于低成本发现可见活动，详情按 `activityId` 查询单项并在不存在时明确返回 `404`。前端不应为了详情把整份活动列表下载后自行筛选。
- `serverTime` 应由后端统一提供，前端倒计时可据此校准，避免用户电脑时间不准导致“活动是否开始”的展示错误；但最终资格申请是否允许，仍由后端状态机和库存脚本裁决。
- `stockStatus: AVAILABLE` 只是列表展示时的库存提示，不能当作资格一定能获得的承诺。并发下最终结果必须以资格申请接口的原子校验为准。
- BFF 路径 `/api/backend/...` 与后端领域路径 `/api/...` 不同。前者负责携带 HttpOnly 会话并转发，前端页面不直接存取或暴露 Bearer Token。
- “资格申请成功”不等于“订单创建成功”。页面和接口状态必须把预约/资格、订单、支付三类事实分开，避免把营销资格误导成交易完成。

### 今天完成

- 用户完成活动公开列表与详情的 Mapper、Service、VO、Controller 读取链路，并保留既有商家创建/预热/取消和消费者资格查询能力。
- Agent 完成商家营销真实接线、消费者活动列表与详情、资格恢复与终态轮询；演示账号不展示为真实可操作活动。
- 后端与前端在 `feature/integration` 合并，分别保留后端提交 `b19e3c1`、前端提交 `f2ff70d`，合并提交为 `eae1b9c`、`07a7535`。
- 验证后端编译、前端 lint/类型检查、4 项测试、生产构建，以及公开活动 BFF 转发、页面 HTTP `200` 与未登录资格查询 `401`。

### 遇到的问题与处理

| 问题 | 原因与处理 | 是否已理解 |
|---|---|---|
| 为什么列表页首次探测超时、复测却成功 | Next.js 开发服务首次编译页面时超过了短超时；使用更长时限复测后 `/promotions` 和 `/promotions/25` 均为 HTTP `200`。不能把首次开发编译等待误判为业务接口故障。 | 是 |
| 为什么活动 25 是 `ACTIVE` 却不能再做预热 | 预热只允许活动处于 `SCHEDULED`。状态迁移是单向业务约束，不能为了验证接口而把已开始的活动倒退。 | 是 |
| 为什么没有手动点击资格申请也能写“集成通过” | 本轮验证了真实公开读取、BFF、页面、鉴权和构建，但避免写入新测试数据。因此结论限定为已验收范围，不把未执行的登录态写入路径写成通过。 | 是 |
...
## Day 34：2026-09-06 / A1 受控经营查询与固定预期测试

### 今天学了什么

- Mapper 内部 `record` 是 SQL 投影的类型安全承接对象，只在 Mapper 与 Service 之间传递原始聚合结果；最终 VO 则是 Controller、前端和未来 AI 使用的稳定接口合同。
- 客单价不能由前端随意计算。已支付营业额和已支付订单数必须使用完全相同的租户、日期与 `PAID` 状态条件，Service 再统一处理除零、两位小数和 `HALF_UP`。
- 日期查询使用 `[startDate 00:00:00, endDate + 1 day 00:00:00)`，避免依赖 `23:59:59` 的精度假设；经营查询统一限制最多 31 天。
- 热销 SKU 使用订单明细的成交价和数量计算历史金额，按销量、销售额、SKU ID 三层稳定排序；展示名称优先取当前 SKU，缺失时回退订单快照。
- 促销资格的 `ORDER_CREATED` 只表示成功创建订单，不代表订单已支付；售后 `APPROVED` 也不代表真实退款完成。指标名称必须服从数据库能证明的事实。
- 售后申请率使用 `LEFT JOIN` 保留没有售后的已支付订单明细，并通过 `COUNT(DISTINCT order_item_id)` 防止同一明细被重复计算。
- 受控查询 Service 是未来 AI 工具调用的安全边界：AI 只能调用有限方法，不能传 tenantId、任意列名或任意 SQL。

### 今天完成

- 用户完成 `merchant.analytics` 的四个 VO、Mapper 聚合 SQL、Service 参数校验/比率计算和四个只读 Controller 接口。
- Agent 完成指标字典、专用测试租户与 SQL 夹具、7 项固定预期集成测试，以及前端 Dashboard 的新增指标接线和 A2 常问问题预告。
- 后端固定预期覆盖商家 A 的 `3` 个有效订单、`2` 个已支付订单、`350.00` 营业额、`175.00` 客单价、热销 SKU、`0.50` 促销转化率和 `0.33` 售后申请率，并单独验证商家 B 数据不会进入 A 的结果。
- 后端提交 `d09e438`、前端提交 `6ad2ade` 均已推送；集合分支通过 `94497d5`、`d53bf55` 顺序合入两边。
- 前端静态检查、4 项测试和生产构建通过；集合分支浏览器真实会话验收仍待补。

### 遇到的问题与处理

| 问题 | 原因与处理 | 是否已理解 |
|---|---|---|
| 第一次运行 A1 测试显示 1 个失败、7 个跳过 | Docker Desktop 未启动，`3307` 和 `13307` 均未监听，Spring 在创建数据源时出现 `SQL State 08S01 / Communications link failure`；启动 Docker/MySQL 后进入下一阶段 | 是 |
| MySQL 恢复后测试仍在 Class Configuration 失败 | A1 查询不依赖 RabbitMQ，但完整 Spring 上下文自动启动消费者，现有 MQ 凭据不匹配导致 `AmqpAuthenticationException`；仅在该测试类关闭 simple/direct listener 自动启动，未修改生产配置 | 是 |
| 为什么测试使用专用租户而不是 1001/1002 | 低库存是当前快照，不受日期限制；复用日常租户会被本机已有商品污染。使用独立租户与 ID 段才能保证固定预期可重复 | 是 |
| 为什么不能只断言接口成功或列表非空 | A1 的目标是让手工 SQL、查询服务和页面数字可复算；固定数值才能发现状态条件、租户条件、聚合或舍入规则发生回归 | 是 |

### 侧边任务/对话补充记录

- 新建 Java 文件时 IDEA 曾先暂存空类、随后工作区改成 record，Git 显示 `AM`；提交前重新 `git add` 才能把最终内容放入暂存区。
- Agent 一度在 `feature/backend` 提前实现前端。发现不符合“后端先提交、再切前端分支”的协作顺序后，已将 `web/` 恢复到零差异；切换 `feature/web-v2` 后重新实现、验证并独立提交。
- `feature/backend` 和 `feature/web-v2` 可以基于同一提交独立开发；最终由 `feature/integration` 先合后端合同、再合前端接线。前端分支不必先包含后端源码才能按已确认的 HTTP 合同开发。
